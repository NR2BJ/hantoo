...
## 2026-03-16 JST | Claude Code (Opus 4.6)

- 범위: 전체 API 필드 매핑 검증 및 수정 (Round 9)
- 무엇을 했는지:
  1. **종목명 미표시 근본 원인 발견**: `inquire-price` API(FHKST01010100)는 종목명 필드(`hts_kor_isnm`)가 아예 없음. 103개 필드 중 이름 필드 자체가 미존재
  2. **Naver 기반 이름 조회 추가**: `_resolve_stock_name()` 함수 — Naver 자동완성으로 심볼→이름 변환, 24시간 캐시
  3. **등락률 순위 데이터 미출력 수정**: fluctuation API는 `stck_shrn_iscd` 사용 (다른 순위 API는 `mksc_shrn_iscd`). `_parse_rank_items`가 `mksc_shrn_iscd`만 확인해서 모든 등락률 데이터 누락
  4. **순위 번호 0 수정**: near-highlow, foreign-institution API는 `data_rank` 필드 미제공 → 인덱스 위치를 fallback으로 사용
  5. **에러 로깅 강화**: `logger.warning` → `logger.exception` (full traceback 표시)
  6. **진단 엔드포인트 추가**: `/api/debug/test-quote` — KIS API 연결/토큰/응답 한번에 테스트
- 어떻게 수정했는지:
  - `_parse_rank_items()`: `mksc_shrn_iscd` or `stck_shrn_iscd` 이중 탐색, `data_rank` or 인덱스 fallback
  - `get_current_price()`: `hts_kor_isnm` 대신 `_resolve_stock_name()` 호출
  - `market.py`: except Exception → logger.exception
- 수정 파일:
  - backend/app/services/quote_service.py — 이름 조회 로직
  - backend/app/services/ranking_service.py — 심볼/순위 필드 호환성
  - backend/app/routers/market.py — exception 로깅 개선
  - backend/app/main.py — debug endpoint 추가
- 검증: KIS API 레퍼런스 docs 전수 대조 완료. 커밋 7878c8f, 5a9c447 푸시 완료
- 메모:
  - KIS API 필드명 차이 정리:
    - inquire-price: 종목명 없음, 다른 API(search-stock-info)에서 `prdt_abrv_name`으로 제공
    - fluctuation: `stck_shrn_iscd` (다른 순위 API: `mksc_shrn_iscd`)
    - near-highlow, foreign-institution: `data_rank` 필드 없음
  - 배당률(`divi_rate`) 주의: 액면가 기준 배당률 (삼성전자: 액면가 100원, 배당 566원 → 566%). 시가 기준 배당수익률(yield)이 아님
  - `acml_vol=0`은 장전/마감 시 정상 (누적 거래량이 해당 세션 기준)

## 2026-03-16 | Claude Code (Opus 4.6)

- 범위: 프론트엔드 디버깅 + 오픈소스 비교 + 파라미터 대소문자 통일
- 무엇을 했는지:
  1. 프론트엔드에 API 응답 디버깅 로그 추가 — api-client.ts에서 모든 응답을 콘솔에 출력
  2. 종목 상세 페이지 모든 탭(Chart, Finance, Dividend, News, Opinion, Investor, Info)에 data/error 콘솔 로깅 추가
  3. 오픈소스 KIS API 프로젝트 조사: python-kis(Soju06), pykis(pjueon), mojito, 공식 open-trading-api
  4. **파라미터 대소문자 통일**: python-kis 및 공식 예제 모두 대문자 사용. finance_service.py와 ranking_service.py에 소문자 파라미터가 혼재 → 전부 대문자로 수정
- 어떻게 수정했는지:
  - api-client.ts: response.json() 후 console.log(`[API] ${shortPath}`, preview) 추가
  - page.tsx: 각 탭 컴포넌트에 `console.log([TabName] data=, error=)` 추가
  - finance_service.py: `fid_cond_mrkt_div_code` → `FID_COND_MRKT_DIV_CODE`, `fid_input_iscd` → `FID_INPUT_ISCD` 등 (income, balance, ratio, opinion 4개 엔드포인트)
  - ranking_service.py: fluctuation과 near-highlow의 소문자 파라미터 → 대문자로 통일
- 수정 파일:
  - frontend/src/lib/api-client.ts — API 응답 디버깅
  - frontend/src/app/(app)/market/[symbol]/page.tsx — 탭별 데이터 디버깅
  - backend/app/services/finance_service.py — 파라미터 대소문자 수정
  - backend/app/services/ranking_service.py — 파라미터 대소문자 수정
- 검증: next build 성공
- 메모:
  - 오픈소스 비교 결과: python-kis는 재무제표 API 미구현. 시세/호가 필드매핑은 우리와 동일
  - KIS API 파라미터는 공식적으로 대문자(FID_*). 소문자도 동작하는 경우가 있지만 규격 불일치
  - **다음 단계**: 브라우저 콘솔에서 [API], [StockDetail], [FinanceTab] 등의 로그로 데이터 흐름 추적. 백엔드 데이터가 정상인지, 프론트 렌더링에서 누락인지 확인 필요
  - Docker 미설치 환경이므로 배포는 사용자가 직접 해야 함

## 2026-03-16 15:30 JST | Claude Code

- 범위: 백엔드 ranking_service + 프론트 market page
- 무엇을 했는지:
  1. 외인/기관 API: FID_INPUT_ISCD "0000"→"0001"(코스피) 수정. 로그에 "Ranking first row keys" 미출력 = output 빈 배열이었음
  2. 52주 신고/저가: change_rate를 prdy_ctrt → hprc_near_rate/lwpr_near_rate 로 변경 (근접률이 이 랭킹의 올바른 지표)
  3. 투자자 데이터: 값 로깅 추가 (date, buy/sell vol for 개인/외인/기관). 실제 0인지 파싱 오류인지 확인용
  4. 프론트 RankTable: rateLabel prop 추가, 52주 신고/저가→"근접률" 헤더
  5. 배당 테이블: duplicate key fix (key={row.year} → key={`${row.year}-${i}`})
- 어떻게 수정했는지:
  - _parse_rank_items()에 change_rate_key 파라미터 추가
  - get_near_highlow()에서 sort에 따라 hprc_near_rate / lwpr_near_rate 전달
  - get_foreign_institution()에서 FID_INPUT_ISCD를 시장별로 정확히 매핑
- 수정 파일:
  - backend/app/services/ranking_service.py
  - frontend/src/app/(app)/market/page.tsx
  - frontend/src/app/(app)/market/[symbol]/page.tsx (duplicate key fix)
- 검증: Python syntax OK. 로컬에 node 없어 프론트 빌드는 Docker에서 확인 필요
- 메모:
  - 투자자 데이터 0 문제: inquire-investor API가 장중에 output[0]에 당일 데이터를 주는데, 실제 값이 0인지 확인 필요. 로그 배포 후 확인할 것
  - 외인/기관: "0000"(전체)이 빈 응답 반환한 것으로 추정. "0001"(코스피) 변경으로 해결 예상
  - 52주 근접률: hprc_near_rate가 100에 가까울수록 52주 최고가에 가까움

## 2026-03-16 15:30 JST | Claude Code

- 범위: backend ranking 디버깅 일괄 수정 + 진단 엔드포인트
- 무엇을 했는지:
  1. 등락률 sort mapping 수정: frontend sort=1(상승)→KIS "0", sort=2(하락)→KIS "1"
  2. 외인/기관 FID_INPUT_ISCD를 "0000"(전체)로 복원 (로그에서 정상 확인됨)
  3. fluctuation/near-highlow 파라미터 대문자로 통일 (이미 적용돼있던 것 재확인)
  4. `/api/ranking/diagnostics` 진단 엔드포인트 추가 — 모든 랭킹 API를 한번에 호출하여 건수/샘플/에러 반환
- 어떻게 수정했는지:
  - ranking_service.py: `"FID_RANK_SORT_CLS_CODE": "0" if sort == "1" else "1"` 매핑 추가
  - ranking_service.py: foreign FID_INPUT_ISCD를 "0000"으로 복원
  - ranking.py: diagnostics 엔드포인트 추가 (volume, fluctuation up/down, market_cap, interest, highlow high/low, foreign, investor_005930 일괄 테스트)
- 수정 파일:
  - backend/app/services/ranking_service.py
  - backend/app/routers/ranking.py
- 검증: git push 후 Portainer 재배포 필요. 배포 후 `/api/ranking/diagnostics` 호출하면 모든 API 상태를 한번에 확인 가능
- 메모:
  - Docker Desktop이 꺼져있어 로컬 빌드 불가. Portainer에서 git pull → 재배포 필요
  - diagnostics 엔드포인트로 향후 API 문제 디버깅 시간을 대폭 단축 가능
  - 투자자 데이터: 장 종료 후에만 당일 데이터 제공됨 (KIS 문서 확인). 장중에는 정상적으로 0
