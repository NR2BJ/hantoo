...
function formatCandles(candles: Candle[]) {
...
function formatVolumes(candles: Candle[]) {
  return candles.map((c) => {
    const d = c.date;
    const time = `${d.slice(0, 4)}-${d.slice(4, 6)}-${d.slice(6, 8)}`;
    // Korean convention: red = up, blue = down
    const color = c.close >= c.open ? "rgba(239, 68, 68, 0.5)" : "rgba(59, 130, 246, 0.5)";
...
export default function StockChart({ symbol, height = 400 }: StockChartProps) {
  ...
  const { data: candles } = useCandles(symbol, period);
